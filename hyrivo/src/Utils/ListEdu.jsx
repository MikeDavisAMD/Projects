            ...
            {education.map((data,index) => (
              (
                <Box key={index} sx={{m:1,p:1,background:COLORS.secondaryBg,borderRadius:2}}>
                  <Box component='span' sx={{fontWeight:'bolder',fontSize:{lg:22,md:22,sm:20,xs:18}}}>{data.college}</Box><br />
                  <Box component='span' sx={{fontSize: {lg:15,md:15,sm:12,xs:13}, color:COLORS.secondaryText}}>{data.Degree}</Box>
                  <Box component="span" sx={{ display: 'inline-block', mx: '2px', transform: 'scale(0.8)' }}>
                    ,
                  </Box>
                  <Box component='span' sx={{fontSize: {lg:15,md:15,sm:12,xs:13}, color:COLORS.secondaryText}}>{data.Field}</Box><br />
