import { Accordion, AccordionActions, AccordionDetails, AccordionSummary, Avatar, Box, Button, Card, Link, Paper, Table, TableBody, TableCell, TableContainer, TableRow, TextField, Typography } from '@mui/material'
...
export const FilledForm = () => {
  ...
  const handleUpdate = async (id) => {
    try {
      await axios.put(`http://localhost:2000/data/${id}`,{
        fname: fname,
        lname: lname,
        email: email,
        mobile: mobile,
  ...
  const fetchData = async () => {
    try {
        ...
        const datas = await Promise.all(
            id.map(id=>axios.get(`http://localhost:2000/data/${id}`))
        )
      ...
      <Card sx={{width:'30%',mb:2}}>
        <Accordion sx={{background:'linear-gradient(#00E6FF,#7700FF)'}}>
          ...
          >
            ...
            <Avatar alt={data.file?.filename || data.fname} src={data.file?.filepath ? `http://localhost:2000/data/${data.file.filepath.replace('\\','/')}` : ''}/>
          ...
          <AccordionActions>
            {edit === data._id ? (
              <Button variant='contained' size='small' startIcon={<Save />} onClick={()=>handleUpdate(data._id)}>Save</Button>
            ) : (
              <Button variant='contained' size='small' startIcon={<Edit />} onClick={()=>handleEdit(data)}>Edit</Button>
            ) }
            <Button variant='contained' size='small' color='error' startIcon={<Delete />} onClick={()=>handleDelete(data._id)}>Delete</Button>
        </AccordionActions>
        </Accordion>
      </Card>
    </Box>
